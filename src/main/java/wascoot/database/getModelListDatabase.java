...
public final class getModelListDatabase {
     ...
     */
    private static final String STATEMENT = "SELECT name, brand, battery_life, weight, height, length, depth, rate_per_minute, rate_per_model FROM public.model";
        ...
        try {
            ...
            while (rs.next()) {
                models.add(new Model(rs.getString("name"), rs
                        ...
                        rs.getInt("depth"), rs.getInt("rate_per_minute"),rs.getInt("rate_per_model")));
            }
        } finally {
            if (rs != null) {
                rs.close();
