...
public class CustomerDAO extends AbstractDAO<List<Customer>>{
    ...
    /**
     * Lists all the customers gender distribution in the database.
     *
     * @return a list of {@code integer} object.
     *
     * @throws SQLException
     *             if any error occurs while searching for customer ages.
     */
    public List<Integer> getCustomerGender() throws SQLException {
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        List<Integer> genders = new ArrayList<>();

        try {
            String statement = "SELECT birthdate FROM public.Customer WHERE postalCode = ?";
            pstmt = con.prepareStatement(statement);
            rs = pstmt.executeQuery();
